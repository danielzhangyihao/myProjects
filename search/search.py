...
def uniformCostSearch(problem):
    ...
    while (1):
        ...
        if newState[2] not in closedList:
            closedList.append(newState[2])   
            for x in problem.getSuccessors(newState[2]):
                fringe.push([newState[0]+[str(x[1])],newState[1]+x[2],x[0]],newState[1]+x[2])
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while (1):
        ...
        if newState[2] not in closedList:
            closedList.append(newState[2])   
            for x in problem.getSuccessors(newState[2]):
                fringe.push([newState[0]+[str(x[1])],newState[1]+x[2],x[0]],heuristic(x[0],problem)+newState[1]+x[2])
