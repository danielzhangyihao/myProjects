...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals)!=len(vals):
            raise SchemeError("number of formals not equal to number of arguments")
        i=0
        while i<=len(vals)-1:
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target[0],do_lambda_form(new_vals, env))
        return target[0]
    else:
...
def do_quote_form(vals):
    ...
    if vals is nil:
       return None
        ...
        return read_line(str(vals[0]))
    elif isinstance(vals.second, Pair):
        return read_line(str(vals[0]+do_quote_form(vals.second)))
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    if scheme_false(scheme_eval(vals[0],env)) and len(vals)>2:
        return vals[2]
    elif scheme_false(scheme_eval(vals[0],env)) and len(vals)==2:   
        return okay
    else:
        return vals[1]
...
def do_or_form(vals, env):
    ...
    if len(vals)==0:
        return False
    for elem in vals:
        if not scheme_false(scheme_eval(elem,env)):
            return elem
    return False
