...
def tweet_time(tweet):
    ...
    return extract_words(tweet['time'])
...
def extract_words(text):
    ...
    """ 
    replace=tuple(map(lambda x: x if x in ascii_letters else ' ', text))
    from functools import reduce
    new_tuple=reduce(lambda a,b: a+b,replace)   
    return new_tuple.split()
...
def make_sentiment(value):
    ...
    0.2
    """
    assert value is one or (value >= -1 and value <= 1), 'Illegal value'
    return value 
...
def analyze_tweet_sentiment(tweet):
    ...
    words_list=tweet_words(tweet)
    counter=0
    sum=0
    for elem in words_list:
        if has_sentiment(get_word_sentiment(elem)):
            sum+=sentiment_value(get_word_sentiment(elem))
            counter+=1
        if counter!=0:
            average=make_sentiment(sum/counter)
    return average
...
def find_centroid(polygon):
    ...
    if A==0:
        ...
        while i < (len(polygon)-1):
            a=latitude(polygon[i])*longitude(polygon[i+1])-latitude(polygon[i+1])*longitude(polygon[i])
            b=latitude(polygon[i])+latitude(polygon[i+1])
            c=longitude(polygon[i])+longitude(polygon[i+1])
            Cx=Cx+k*a*b
            Cy=Cy+k*a*c
            i+=1
        return (abs(Cx),abs(Cy),abs(A))
...
def group_tweets_by_state(tweets):
    ...
    while i<=len(tweets)-1:
        ...
        if closest_state in tweets_by_state:
            tweets_by_state[closest_state].append(tweet_string(tweet))
        else:
            tweets_by_state[closest_state]=[tweet_string(tweet)]
        i+=1
...
def average_sentiments(tweets_by_state):
    ...
    for elem in tweets_by_state:
            counter=0
            sum=0
            for x in tweets_by_state[elem]:
                if has_sentiment(analyze_tweet_sentiment(x)):
                    sum+=(sentiment_value(analyze_tweet_sentiment(x))
                    counter+=1
            if counter != 0:
                average=sum/counter
